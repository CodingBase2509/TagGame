...
/// Defaults are conservative and can be overridden via configuration.
/// </summary>
public sealed class NetworkResilienceOptions
{
    ...
    public sealed class HttpOptions
    {
        ...
        /// <summary>Maximum delay cap in milliseconds between retries.</summary>
        public int MaxDelayMs { get; init; } = 2000;

        /// <summary>Total request timeout across all tries in milliseconds.</summary>
        public int TotalRequestTimeoutMs { get; init; } = 20_000;

        /// <summary>Retry HTTP 429 responses (respecting Retry-After) when true.</summary>
        public bool RetryOn429 { get; init; }

        /// <summary>Allow retries for non-idempotent methods (POST/PUT/PATCH/DELETE) when true.</summary>
        public bool RetryOnNonIdempotent { get; init; }
    }

    public sealed class HubOptions
    {
        /// <summary>Reconnect delays in milliseconds for SignalR automatic reconnect.</summary>
        public int[] ReconnectDelaysMs { get; init; } = [0, 1000, 2000, 5000, 10_000, 30_000];

        /// <summary>Maximum reconnect window in milliseconds before giving up.</summary>
